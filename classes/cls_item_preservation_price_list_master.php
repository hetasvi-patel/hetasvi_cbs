...
{   
    ...
    public function __construct() {
        ...
        global $tbl_item_preservation_price_list_master;
        $select = $_dbh->prepare("SELECT `generator_options` FROM `{$tbl_generator_master}` WHERE `table_name` = ?");
        $select->bindParam(1,  $tbl_item_preservation_price_list_master);
        $select->execute();
...
{   
    ...
    /** \FOR DETAIL **/
       ...
       {
            header("Location:../frm_item_preservation_price_list_master.php"); // URL CHANGED BY BHUMITA ON 30/07/2025
       }
      ...
      public function pageSearch()
    {
       ...
       //ADDED BY HETASVI ON 26/09/25
        $company_year_query = COMPANY_YEAR_QUERY;
        $sql="CAll csms_search_detail('t.*, t1.item_name as item_id, u1.person_name as created_by, u2.person_name as modified_by, c1.company_name as company_id','tbl_item_preservation_price_list_master t LEFT JOIN tbl_item_master  t1 ON t.item_id=t1.item_id LEFT JOIN tbl_user_master u1  ON t.created_by=u1.user_id  LEFT JOIN tbl_user_master u2  ON t.modified_by=u2.user_id  LEFT JOIN tbl_company_master c1  ON t.company_id=c1.company_id ','".$company_query." ".$company_year_query."')";//ADDED $company_year_query BY HETASVI ON 26/09/25
        echo "<!-- Filter row -->
        ...
        if($canUpdate) {
        $_grid.="<form  method=\"post\" action=\"frm_item_preservation_price_list_master.php\" style=\"display:inline; margin-rigth:5px;\">
        ...
        if($canDelete) { 
        $_grid.="<form  method=\"post\" action=\"classes/cls_item_preservation_price_list_master.php\" style=\"display:inline;\">
    ...
    public function checkMasterRecord() {
        global $_dbh, $tbl_item_preservation_price_list_master;
        ...
        $sql = "SELECT * FROM {$tbl_item_preservation_price_list_master} 
                WHERE item_id = ? ".COMPANY_QUERY.COMPANY_YEAR_QUERY;
        $stmt = $_dbh->prepare($sql);
        $stmt->execute([$itemId]);
 ...
 class dal_itempreservationpricelistmaster                         
{
    public function dbTransaction($_mdl)                     
    {
        ...
        try {
            if($_mdl->_item_preservation_price_list_id=="") {
                $_mdl->_item_preservation_price_list_id=0;
            }
            ...
            $_pre=$_dbh->prepare("CALL item_preservation_price_list_master_transaction (@p0,?,?,?,?,?,?,?,?,?,?) ");
                ...
                if($_mdl->_ajaxAdd==1) {
                    $_mdl->_ajaxAdd=0;
                    if($_mdl->_item_preservation_price_list_id)
                        $return_id=$_mdl->_item_preservation_price_list_id;
                    else 
                        $return_id=0;
                    echo $return_id;
                    exit;
                ...
                errorHandling($e, "item_preservation_price_list_id", $tbl_item_preservation_price_list_master, $ajax, $_mdl->_item_preservation_price_list_id);
    ...
    if(is_array($_bll->_mdl->generator_fields_names) && !empty($_bll->_mdl->generator_fields_names)){
        ...
        foreach($_bll->_mdl->generator_fields_names as $i=>$fieldname)
        {
            ...
            if($_bll->_mdl->generator_fields_types[$i]=="file") {
                $upload_dir = UPLOAD_DIR ."item_preservation_price_list_master/";
                $upload_path = UPLOAD_PATH ."item_preservation_price_list_master/";
                $url_fieldname = $fieldname . "_url";
