...
{   
     ...
     public function pageSearch()
    {
        ...
        $sql="CAll csms_search_detail('t.*, t6.city_name as city_id, s.state_name as state_id, c.country_name as country_id, t18.value as status, u1.person_name as created_by, u2.person_name as modified_by, c1.company_name as company_id','tbl_customer_master t LEFT JOIN tbl_city_master t6 ON t.city_id=t6.city_id LEFT JOIN tbl_state_master s ON t.state_id=s.state_id LEFT JOIN tbl_country_master c ON t.country_id=c.country_id LEFT JOIN view_status_type t18 ON t.status=t18.id LEFT JOIN tbl_user_master u1  ON t.created_by=u1.user_id  LEFT JOIN tbl_user_master u2  ON t.modified_by=u2.user_id  LEFT JOIN tbl_company_master c1  ON t.company_id=c1.company_id ','".$company_query."')"; // STATE NAME AND COUNTRY NAME FETCH FUNCTIONALITY ADDED IN THIS QUERY BY BHUMITA ON 22/07/2025
        echo "<!-- Filter row -->
        ...
        if($canUpdate) {
        $_grid.="<form  method=\"post\" action=\"frm_customer_master.php\" style=\"display:inline; margin-rigth:5px;\">
        ...
        if($canDelete) { 
        $_grid.="<form  method=\"post\" action=\"classes/cls_customer_master.php\" style=\"display:inline;\">
    ...
    public function fetchCountryAndState() {
        ...
        $columns = "c.country_id, c.country_name, s.state_id, s.state_name";
        ...
        $whereCondition = " and ct.city_id = $city_id";
    try {
            $stmt = $_dbh->prepare("CALL csms_search_detail(:columns, :tableName, :whereCondition)");
    ...
    public function getForm($transactionmode="I",$popup=false,$label_classes="col-12 col-sm-3 col-md-2 col-lg-2 col-xl-2 col-xxl-1", $field_classes="col-12 col-sm-6 col-md-4 col-lg-4 col-xl-3 col-xxl-2") {
        $output=""; $hidden_str="";
        /* ADDED BY BHUMITA ON 13/07/2025 */
        $city_menu_id = getMenuId("frm_city_master.php");
        $canAddCity = userHasRight(USER_ID, $city_menu_id, 'add'); 
        /* \ADDED BY BHUMITA ON 13/07/2025 */
        ...
        if(is_array($this->_mdl->generator_fields_names) && !empty($this->_mdl->generator_fields_names)){
            ...
            foreach($this->_mdl->generator_fields_names as $i=>$fieldname)
            {
                ...
                if($fieldname=="state_name" || $fieldname=="country_name") {
                    continue; 
                }
                /* \ADDED BY BHUMITA ON 22/07/2025 */
                ...
                $add_modal_cls="";  //ADDED BY BHUMITA 22/07/2025
                ...
                /* ADDED BY BHUMITA ON 22/07/2025 */
                if ($fieldname == "gstin" || $fieldname == "pan") {
                    $error_container = '<div class="invalid-feedback"></div>';
                }
                /* \ADDED BY BHUMITA ON 22/07/2025 */
                $lbl_str.="</label>";
                switch($this->_mdl->generator_fields_types[$i]) {
                    ...
                    case "number":
                    case "select":
                        $value="";$field_str="";$cls="";$flag=0;$disabled_value=""; // $disabled_value added by BHUMITA on 22/07/2025
                            ...
                            if($this->_mdl->generator_fields_types[$i]=="number") {
                                ...
                                if(!empty($this->_mdl->generator_dropdown_table[$i]) && !empty($this->_mdl->generator_label_column[$i]) && !empty($this->_mdl->generator_value_column[$i])) {
                                    ...
                                    /* ADDED BY BHUMITA 07/07/2025 */
                                    if(!$popup && $canAddCity && $fieldname=="city_id") {
                                        $add_modal_cls="add_modal_icon";
                                        $field_str.='<i class="bi bi-plus" data-bs-toggle="modal" data-bs-target="#addCityModal" title="Add City"></i>';
                                    }
                                    /* \ADDED BY BHUMITA 07/07/2025 */
                                }
                            } else {
                                if($flag==0) {
                                    ...
                                    /* ADDED BY BHUMITA 22/07/2025 */
                                    if($fieldname=="state_id" && $transactionmode=="U" && $this->_mdl->_state_name!="") {
                                        $disabled_value=$this->_mdl->_state_name;
                                    }
                                    if($fieldname=="country_id" && $transactionmode=="U" && $this->_mdl->_country_name!="") {
                                        $disabled_value=$this->_mdl->_country_name;
                                    }
                                    /* \ADDED BY BHUMITA 22/07/2025 */
                                    $field_str.=addInput($this->_mdl->generator_fields_types[$i],$fieldname,$value,$required_str,$disabled_str,$cls,$duplicate_str,$chk_str,$this->_mdl->generator_fields_labels[$i],$disabled_value).$error_container.$img_str; //Last parameter $disabled_value added by BHUMITA on 22/07/2025
                                }
                            }
                        break;
                    case "hidden":
                 ...
                 if(empty($this->_mdl->generator_after_detail) || (!empty($this->_mdl->generator_after_detail) && !in_array($fieldname,$this->_mdl->generator_after_detail))) {
                    ...
                    $output.=$lbl_str;
                    if($field_str) {
                    $output.='<div class="'.$field_layout_classes.' '.$add_modal_cls.'">'; //$add_modal_cls ADDED BY BHUMITA 22/07/2025
                    $output.=$field_str;
                    $output.='</div>';
                    }
                    if($table_layout=="vertical" && $this->_mdl->generator_fields_types[$i]!="hidden") {
 ...
 class dal_customermaster                         
{
    public function dbTransaction($_mdl)                     
    {
        ...
        try {
            ...
            $_pre=$_dbh->prepare("CALL customer_master_transaction (@p0,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ");
                ...
                errorHandling($e,"customer_id", $tbl_customer_master, $ajax,$_mdl->_customer_id);
    ...
    public function fillModel($_mdl)
    {
        ...
        if(!empty($_rs)) {
            ...
            $_mdl->_state_name = $_rs[0]['state_name'] ?? ''; // ADDED BY BHUMITA ON 22/07/2025
            $_mdl->_country_name = $_rs[0]['country_name'] ?? ''; // ADDED BY BHUMITA ON 22/07/2025
    ...
    if(is_array($_bll->_mdl->generator_fields_names) && !empty($_bll->_mdl->generator_fields_names)){
        ...
        foreach($_bll->_mdl->generator_fields_names as $i=>$fieldname)
        {
            ...
            if($_bll->_mdl->generator_fields_types[$i]=="file") {
                $upload_dir = UPLOAD_DIR ."customer_master/";
                $upload_path = UPLOAD_PATH ."customer_master/";
                $url_fieldname = $fieldname . "_url";
            ...
            if(isset($_REQUEST[$fieldname]) && !empty($_REQUEST[$fieldname])) {
                ...
                } else if($_bll->_mdl->generator_fields_types[$i]=="checkbox" && $_bll->_mdl->generator_dropdown_table!="" && $_bll->_mdl->generator_label_column!="" && $_bll->_mdl->generator_value_column!="" && is_array($field)) {
                    $field = implode(',', $field);
                }
                if(!is_array($field))
                    $field=trim($field);
