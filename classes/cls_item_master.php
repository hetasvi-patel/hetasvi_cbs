...
{   
    ...
    public function __construct()    
    {
        $this->_mdl =new mdl_itemmaster(); 
        $this->_dal =new dal_itemmaster();
    }
    ...
    public function dbTransaction()
    {
       ...
       {
            header("Location:../srh_item_master.php");
       }
       ...
       {
            header("Location:../frm_item_master.php");
       }
     ...
     public function pageSearch()
    {
        ...
        $sql="CAll csms_search_detail('t.*, t1.value as item_gst, t4.value as status, u1.person_name as created_by, u2.person_name as modified_by, c1.company_name as company_id','tbl_item_master t LEFT JOIN view_item_gst_type t1 ON t.item_gst=t1.id LEFT JOIN view_status_type t4 ON t.status=t4.id LEFT JOIN tbl_user_master u1  ON t.created_by=u1.user_id  LEFT JOIN tbl_user_master u2  ON t.modified_by=u2.user_id  LEFT JOIN tbl_company_master c1  ON t.company_id=c1.company_id ','".$company_query."')";
        echo "<!-- Filter row -->
        ...
        if($canUpdate) {
        $_grid.="<form  method=\"post\" action=\"frm_item_master.php\" style=\"display:inline; margin-rigth:5px;\">
            <i class=\"fa fa-edit update\" style=\"cursor: pointer;\"></i>
            <input type=\"hidden\" name=\"item_id\" value=\"".$_rs["item_id"]."\" />
            <input type=\"hidden\" name=\"transactionmode\" value=\"U\"  />
            </form>";
        }
        if($canDelete) { 
        $_grid.="<form  method=\"post\" action=\"classes/cls_item_master.php\" style=\"display:inline;\">
            <i class=\"fa fa-trash delete custom-delete\" style=\"cursor: pointer;\"></i>
            <input type=\"hidden\" name=\"item_id\" value=\"".$_rs["item_id"]."\" />
 ...
 class dal_itemmaster                         
{
    public function dbTransaction($_mdl)                     
    {
        ...
        try {
            ...
            $_dbh->exec("set @p0 = ".$_mdl->_item_id);
            $_pre=$_dbh->prepare("CALL item_master_transaction (@p0,?,?,?,?,?,?,?,?,?,?) ");
                ...
                if($_mdl->_transactionmode=="I") {
                    ...
                    $_mdl->_item_id=$insertedId;
                }
                if($_mdl->_ajaxAdd==1) {
                    $_mdl->_ajaxAdd=0;
                    if($_mdl->_item_id)
                        $return_id=$_mdl->_item_id;
                    else 
                        $return_id=0;
                    echo $return_id;
                    exit;
                ...
                errorHandling($e,"item_id",$tbl_item_master, $ajax, $_mdl->_item_id);
    ...
    {
        global $_dbh;
        $_pre=$_dbh->prepare("CALL item_master_fillmodel (?) ");
        $_pre->bindParam(1,$_REQUEST["item_id"]);
        $_pre->execute();
        $_rs=$_pre->fetchAll(); 
        if(!empty($_rs)) {
            if(is_array($_mdl->generator_fields_names) && !empty($_mdl->generator_fields_names)){
    ...
    if(is_array($_bll->_mdl->generator_fields_names) && !empty($_bll->_mdl->generator_fields_names)){
        ...
        foreach($_bll->_mdl->generator_fields_names as $i=>$fieldname)
        {
            ...
            if($_bll->_mdl->generator_fields_types[$i]=="file") {
                $upload_dir = UPLOAD_DIR ."item_master/";
                $upload_path = UPLOAD_PATH ."item_master/";
                $url_fieldname = $fieldname . "_url";
