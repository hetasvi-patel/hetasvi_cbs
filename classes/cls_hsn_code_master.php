...
include("cls_gst_tax_detail.php"); 
        class mdl_hsncodemaster 
{   
...
{   
    ...
    public function __construct()    
    {
        $this->_mdl =new mdl_hsncodemaster(); 
        $this->_dal =new dal_hsncodemaster();
    }
    ...
    public function dbTransaction()
    {
       ...
       /** FOR DETAIL **/
        if($this->_mdl->_hsn_code_id>0) {
            $_bllitem= new bll_gsttaxdetail();
            if($this->_mdl->_transactionmode!="D")
            {
                if(!empty($this->_mdl->_array_itemdetail)) {
                        for($iterator= $this->_mdl->_array_itemdetail->getIterator();$iterator->valid();$iterator->next())
                        {
                            ...
                            $_bllitem->_mdl->hsn_code_id = $this->_mdl->_hsn_code_id;
    ...
    /** \FOR DETAIL **/
       ...
       if($this->_mdl->_transactionmode =="D")
       {
            header("Location:../srh_hsn_code_master.php");
       }
       ...
       {
            header("Location:../frm_hsn_code_master.php");
       }
    ...
    public function pageSearch()
    {
        ...
        $sql="CAll csms_search_detail('t.*, gt.*, tt.value as tax_type, u1.person_name as created_by, u2.person_name as modified_by, c1.company_name as company_id','tbl_hsn_code_master t LEFT JOIN tbl_gst_tax_detail gt ON t.hsn_code_id=gt.hsn_code_id LEFT JOIN view_tax_type tt ON gt.tax_type=tt.id LEFT JOIN tbl_user_master u1  ON t.created_by=u1.user_id  LEFT JOIN tbl_user_master u2  ON t.modified_by=u2.user_id  LEFT JOIN tbl_company_master c1  ON t.company_id=c1.company_id ','".$company_query."')"; // ADDED gt and tt tables BY BHUMITA ON 10/09/2025
        $result=$_dbh->query($sql, PDO::FETCH_ASSOC);

        $_grid.='<div class="row gx-2 gy-1 align-items-center" id="search-filters">';
        $_grid.='<div class="col-auto"><input type="text" class="form-control" placeholder="Search HSN Code" data-index="1" /></div>';
        $_grid.='<div class="col-auto"><input type="text" class="form-control" placeholder="Search Description" data-index="2" /></div>';
        $_grid.='<div class="col-auto"><input type="text" class="form-control" placeholder="Search Tax Type" data-index="3" /></div>';
        $_grid.='<div class="col-auto"><input type="text" class="form-control" placeholder="Search Tax" data-index="4" /></div>';
        $_grid.='<div class="col-auto"><input type="text" class="form-control date-filter" placeholder="Search Effective Date" data-index="5" /></div>';
        $_grid.='<div class="col-auto"><input type="text" class="form-control" placeholder="Search Remarks" data-index="6" /></div>';
        $_grid.='<div class="col-auto"><input type="text" class="form-control" placeholder="Search Modified By" data-index="7" /></div>';
        $_grid.='<div class="col-auto"><input type="text" class="form-control date-filter" placeholder="Search Modified Date" data-index="8" /></div>';
        if(COMPANY_ID==ADMIN_COMPANY_ID) {
             $_grid.='<div class="col-auto"><input type="text" class="form-control date-filter" placeholder="Search Company" data-index="9" /></div>';
         }
         $_grid.='</div>';
         $_grid.='
         ...
         $_grid.="</tr>
            <tbody>
        ";
        $default_codes=['996721','998619'];
        if(!empty($result))
        {
            foreach($result as $_rs)
            {   
                 /* ADDED BY BHUMTIA ON 10/09/2025 TO PREVENT DELETE FOR DEFAULT HSN CODES */
                $j++;
                $_grid.="<tr>";
                ...
                if($canUpdate) {
                    $_grid.="<form  method=\"post\" action=\"frm_hsn_code_master.php\" style=\"display:inline; margin-rigth:5px;\">
                        <i class=\"fa fa-edit update\" style=\"cursor: pointer;\"></i>
                        <input type=\"hidden\" name=\"hsn_code_id\" value=\"".$_rs["hsn_code_id"]."\" />
                        <input type=\"hidden\" name=\"transactionmode\" value=\"U\"  />
                        </form>";
                }
                if($canDelete && !in_array($_rs['hsn_code_name'],$default_codes)) { // ADDED CONDITION TO PREVENT DELETE FOR DEFAULT HSN CODES BY BHUMITA ON 10/09/2025
                    $_grid.="<form  method=\"post\" action=\"classes/cls_hsn_code_master.php\" style=\"display:inline;\">
                ...
                /* ADDED BY BHUMTIA ON 10/09/2025 */
                if($_rs['effective_date']!="") {
                    $effective_date=date("d/m/Y",strtotime($_rs['effective_date']));
                } else {
                    $effective_date="";
                }
                /* \ADDED BY BHUMTIA ON 10/09/2025 */
                $_grid.= "<td data-label=\"HSN Code\"> ".$_rs['hsn_code_name']." </td>";
                $_grid.= "<td data-label=\"Description\"> ".$_rs['description']." </td>";
                $_grid.= "<td data-label=\"Tax Type\"> ".$_rs['tax_type']." </td>";
                $_grid.= "<td data-label=\"Tax(%)\"> ".$_rs['tax']." </td>";
                $_grid.= "<td data-label=\"Effective Date\"> ".$effective_date." </td>";
                $_grid.= "<td data-label=\"Remark\"> ".$_rs['remark']." </td>";
                $_grid.= "<td data-label=\"Modified By\"> ".$_rs['modified_by']." </td>";
                $_grid.= "<td data-label=\"Modified Date\"> ".date("d/m/Y",strtotime($_rs['modified_date']))."<br><small>".date("h:i:s a",strtotime($_rs['modified_date']))."</small></td>";
                if(COMPANY_ID==ADMIN_COMPANY_ID) {
                    ...
                    case "select":
                        $value="";$field_str="";$cls="";$flag=0;
                            ...
                            if($this->_mdl->generator_fields_types[$i]=="number") {
                                ...
                                $min_str='min="'.$min.'"';
                                $field_str.=addNumber($fieldname,$value,$required_str,$disabled_str,$cls,$duplicate_str,$min_str,$step_str,$this->_mdl->generator_fields_labels[$i]).$error_container;
                            }
                ...
                $output.="<!-- detail table content--> 
                        <div class=\"box-body\">
                            <div class=\"box-detail\">";
                                     $_blldetail = new bll_gsttaxdetail();
                                     $detailHtml = $_blldetail->pageSearch();
                                    if($detailHtml)
                                        $output.=$detailHtml; 
                                $output.="<button type=\"button\" name=\"detailBtn\" id=\"detailBtn\" class=\"btn btn-primary add\" data-bs-toggle=\"modal\" data-bs-target=\"#modalDialog\" >Add Detail Record</button>
                        </div>
                    </div>
 ...
 class dal_hsncodemaster                         
{
    public function dbTransaction($_mdl)                     
    {
        ...
        try {
            if($_mdl->_hsn_code_id=="") {
                $_mdl->_hsn_code_id=0;
            }
            $_dbh->exec("set @p0 = ".$_mdl->_hsn_code_id);
            $_pre=$_dbh->prepare("CALL hsn_code_master_transaction (@p0,?,?,?,?,?,?,?,?,?) ");
                ...
                if($_mdl->_transactionmode=="I") {
                    ...
                    $_mdl->_hsn_code_id=$insertedId;
                }
                if($_mdl->_ajaxAdd==1) {
                    $_mdl->_ajaxAdd=0;
                    if($_mdl->_hsn_code_id)
                        $return_id=$_mdl->_hsn_code_id;
                    else 
                        $return_id=0;
                    echo $return_id;
                    exit;
                ...
                errorHandling($e,"hsn_code_id",$tbl_hsn_code_master, $ajax,$_mdl->_hsn_code_id);
    ...
    {
        global $_dbh;
        $_pre=$_dbh->prepare("CALL hsn_code_master_fillmodel (?) ");
        $_pre->bindParam(1,$_REQUEST["hsn_code_id"]);
        $_pre->execute();
        $_rs=$_pre->fetchAll(); 
        if(!empty($_rs)) {
            if(is_array($_mdl->generator_fields_names) && !empty($_mdl->generator_fields_names)){
    ...
    if(is_array($_bll->_mdl->generator_fields_names) && !empty($_bll->_mdl->generator_fields_names)){
        ...
        foreach($_bll->_mdl->generator_fields_names as $i=>$fieldname)
        {
            ...
            if($_bll->_mdl->generator_fields_types[$i]=="file") {
                $upload_dir = UPLOAD_DIR ."hsn_code_master/";
                $upload_path = UPLOAD_PATH ."hsn_code_master/";
            ...
            if(isset($_REQUEST[$fieldname]) && !empty($_REQUEST[$fieldname])) {
                ...
                }else if(($_bll->_mdl->generator_fields_types[$i]=="checkbox" || $_bll->_mdl->generator_fields_types[$i]=="radio") && $_bll->_mdl->generator_dropdown_table!="" && $_bll->_mdl->generator_label_column!="" && $_bll->_mdl->generator_value_column!="") {
                    $field = implode(",", $field);
                }
                $field=trim($field);
            }    
