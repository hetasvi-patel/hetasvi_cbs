...
{   
    ...
    {
        global $_dbh, $tbl_customer_wise_item_preservation_price_list_detail, $tbl_customer_wise_item_preservation_price_list_master, $tbl_item_preservation_price_list_detail, $tbl_item_preservation_price_list_master, $tbl_packing_unit_master;
        $_grid="";
        $_grid.="
        ...
        $company_query3=str_replace("company_id","cpm.company_id",COMPANY_QUERY);
        $company_year_query=str_replace("`company_year_id`","pm.company_year_id",COMPANY_YEAR_QUERY);
       ...
       $sql = "SELECT 
                ...
                COALESCE(cd.rent_per_qty_month, d.rent_per_qty_month, '0.00') AS rent_per_qty_month,
                COALESCE(cd.rent_per_qty_season, d.rent_per_qty_season, '0.00') AS rent_per_qty_season,
                cd.customer_wise_item_preservation_price_list_detail_id
            FROM {$tbl_packing_unit_master} um
            LEFT JOIN (
                    SELECT 
                        cpd.packing_unit_id, 
                        cpd.rent_per_qty_month, 
                        cpd.rent_per_qty_season,
                        cpd.customer_wise_item_preservation_price_list_detail_id
                    FROM {$tbl_customer_wise_item_preservation_price_list_detail} cpd
                    INNER JOIN {$tbl_customer_wise_item_preservation_price_list_master} cpm
                        ON cpd.customer_wise_item_preservation_price_list_id = cpm.customer_wise_item_preservation_price_list_id
                    WHERE cpd.customer_wise_item_preservation_price_list_id = :master_id
                    {$company_query3}
                ) cd ON um.packing_unit_id = cd.packing_unit_id
            LEFT JOIN (
                SELECT 
                    pd.packing_unit_id, 
        ...
        if (!empty($result)) {
            foreach ($result as $_rs) {
                $_grid .= "
                <tr data-id=\"{$_rs['packing_unit_id']}\" data-detail-id=\"" . ($_rs['customer_wise_item_preservation_price_list_detail_id'] ?? '') . "\">
                    <td style=\"background-color: #f0f0f0;\">{$_rs['packing_unit_name']}</td>
                    <td contenteditable=\"true\" class=\"editable rent-monthly\" data-field=\"rent_per_qty_month\" data-original=\"{$_rs['rent_per_qty_month']}\">
                        {$_rs['rent_per_qty_month']}
                    </td>
                    <td contenteditable=\"true\" class=\"editable rent-seasonal\" data-field=\"rent_per_qty_season\" data-original=\"{$_rs['rent_per_qty_season']}\">
 ...
 class dal_customerwiseitempreservationpricelistdetail                         
{
    public function dbTransaction($_mdl)                     
    {
        ...
        $_pre->bindParam(1,$_mdl->customer_wise_item_preservation_price_list_id,PDO::PARAM_INT);
