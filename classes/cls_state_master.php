...
{   
    ...
    public function pageSearch()
    {
        ...
        $sql="CAll csms_search_detail('t.*, t2.country_name as country_id, u1.person_name as created_by, u2.person_name as modified_by, c1.company_name as company_id','tbl_state_master t LEFT JOIN tbl_country_master t2 ON t.country_id=t2.country_id LEFT JOIN tbl_user_master u1  ON t.created_by=u1.user_id  LEFT JOIN tbl_user_master u2  ON t.modified_by=u2.user_id  LEFT JOIN tbl_company_master c1  ON t.company_id=c1.company_id ','".$company_query."')";
        echo "<!-- Filter row -->
        ...
        if($canUpdate) {
        $_grid.="<form  method=\"post\" action=\"frm_state_master.php\" style=\"display:inline; margin-rigth:5px;\">
        ...
        if($canDelete) { 
        $_grid.="<form  method=\"post\" action=\"classes/cls_state_master.php\" style=\"display:inline;\">
    ...
    public function getForm($transactionmode="I",$popup=false,$label_classes="col-12 col-sm-3 col-md-2 col-lg-2 col-xl-2 col-xxl-1", $field_classes="col-12 col-sm-6 col-md-4 col-lg-4 col-xl-3 col-xxl-2") {
        $output=""; $hidden_str="";
        /* ADDED BY BHUMITA ON 07/07/2025 */
        $country_menu_id = getMenuId("frm_country_master.php");
        $canAddCountry = userHasRight(USER_ID, $country_menu_id, 'add'); 
        /* \ADDED BY BHUMITA ON 07/07/2025 */
        ...
        if(is_array($this->_mdl->generator_fields_names) && !empty($this->_mdl->generator_fields_names)){
            ...
            foreach($this->_mdl->generator_fields_names as $i=>$fieldname)
            {
                ...
                $add_modal_cls="";  //ADDED BY BHUMITA 07/07/2025
                ...
                switch($this->_mdl->generator_fields_types[$i]) {
                        ...
                        $value="";$field_str="";$cls="";$flag=0;
                            ...
                            if($this->_mdl->generator_fields_types[$i]=="number") {
                                ...
                                if(!empty($this->_mdl->generator_dropdown_table[$i]) && !empty($this->_mdl->generator_label_column[$i]) && !empty($this->_mdl->generator_value_column[$i])) {
                                    ...
                                    if(!$popup && $canAddCountry && $fieldname=="country_id") {
                                        $add_modal_cls="add_modal_icon";
                                        $field_str.='<i class="bi bi-plus" data-bs-toggle="modal" data-bs-target="#addCountryModal" title="Add Country"></i>';
                                    }
                                    /* \ADDED BY BHUMITA 07/07/2025 */
                                }
                            } else {
                 ...
                 if(empty($this->_mdl->generator_after_detail) || (!empty($this->_mdl->generator_after_detail) && !in_array($fieldname,$this->_mdl->generator_after_detail))) {
                    ...
                    $output.=$lbl_str;
                    if($field_str) {
                    $output.='<div class="'.$field_layout_classes.' '.$add_modal_cls.'">'; //$add_modal_cls ADDED BY BHUMITA 07/07/2025
                    $output.=$field_str;
                    $output.='</div>';
                    }
                    if($table_layout=="vertical" && $this->_mdl->generator_fields_types[$i]!="hidden") {
        ...
        try {
            ...
            $_pre=$_dbh->prepare("CALL state_master_transaction (@p0,?,?,?,?,?,?,?,?,?) ");
                ...
                if($_mdl->_transactionmode=="I") {
                    ...
                    $_mdl->_state_id=$insertedId;
                }
                if($_mdl->_ajaxAdd==1) {
                    $_mdl->_ajaxAdd=0;
                    if($_mdl->_state_id)
                        $return_id=$_mdl->_state_id;
                    else 
                        $return_id=0;
                    echo $return_id;
                    exit;
                ...
                errorHandling($e,"state_id",$tbl_state_master,$ajax,$_mdl->_state_id);
...
if(isset($_POST["masterHidden"]) && ($_POST["masterHidden"]=="save"))
{
    ...
    if(is_array($_bll->_mdl->generator_fields_names) && !empty($_bll->_mdl->generator_fields_names)){
        ...
        foreach($_bll->_mdl->generator_fields_names as $i=>$fieldname)
        {
            ...
            if($_bll->_mdl->generator_fields_types[$i]=="file") {
                $upload_dir = UPLOAD_DIR ."state_master/";
                $upload_path = UPLOAD_PATH ."state_master/";
                $url_fieldname = $fieldname . "_url";
   ...
   // echo "<pre>"; print_r($_bll->_mdl); echo "</pre>"; exit;
 $_bll->dbTransaction();
}
