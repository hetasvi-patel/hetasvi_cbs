...
{   
    ...
    public function __construct() {
        ...
        global $tbl_customer_wise_item_preservation_price_list_master;
        $select = $_dbh->prepare("SELECT `generator_options` FROM `{$tbl_generator_master}` WHERE `table_name` = ?");
        $select->bindParam(1,  $tbl_customer_wise_item_preservation_price_list_master);
        $select->execute();
...
{   
    ...
    /** \FOR DETAIL **/
       ...
       {
            header("Location:../frm_customer_wise_item_preservation_price_list_master.php"); // URL CHANGED BY BHUMITA ON 30/07/2025
       }
      ...
      public function pageSearch()
    {
        ...
        //ADDED BY HETANSHREE ON 26/09/25
        $company_year_query = COMPANY_YEAR_QUERY;

        $sql = "CAll csms_search_detail(
            't.*, t1.customer as customer_id, t2.item_name as item_id, u1.person_name as created_by, u2.person_name as modified_by, c1.company_name as company_id',
            'tbl_customer_wise_item_preservation_price_list_master t
                LEFT JOIN tbl_customer_master t1 ON t.customer_id=t1.customer_id
                LEFT JOIN tbl_item_master t2 ON t.item_id=t2.item_id
            ...
            '".$company_query." ".$company_year_query."'
        )";//ADDED $company_year_query BY HETANSHREE ON 26/09/25
        echo "<!-- Filter row -->
        ...
        if($canUpdate) {
        $_grid.="<form  method=\"post\" action=\"frm_customer_wise_item_preservation_price_list_master.php\" style=\"display:inline; margin-rigth:5px;\">
        ...
        if($canDelete) { 
        $_grid.="<form  method=\"post\" action=\"classes/cls_customer_wise_item_preservation_price_list_master.php\" style=\"display:inline;\">
    ...
    public function checkMasterRecord() {
         global $_dbh, $tbl_customer_wise_item_preservation_price_list_master,$tbl_item_preservation_price_list_master;
        ...
        $sql = "SELECT * FROM {$tbl_customer_wise_item_preservation_price_list_master} 
                ...
                AND customer_id = ? 
                ".COMPANY_QUERY.COMPANY_YEAR_QUERY."
                AND (customer_wise_item_preservation_price_list_id != ? OR ? = 0)";
        
        $stmt = $_dbh->prepare($sql);
        $stmt->execute([$itemId,$customerId,$masterId,$masterId]);
       $currentYearRecord = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($currentYearRecord) {
            $response=['exists' => true, 'data' => $currentYearRecord];
        ...
        $lastCreatedRecord = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($lastCreatedRecord) {
            $response=['exists' => false, 'prevYearData' => $lastCreatedRecord];
            echo json_encode($response);
            exit;
        }
        ...
        if ($lastCreatedRecord) {
            $response=['exists' => false, 'prevItemData' => $lastCreatedRecord];
            echo json_encode($response);
            exit;
        }
        $sql = "SELECT * FROM {$tbl_item_preservation_price_list_master}
            ...
            $output.=$hidden_str;
               $output.="</div> <!-- /.box-body -->";
                ...
                $output.="<!-- detail table content--> 
                        <div class=\"box-body\">
                            <div class=\"box-detail\" id=\"gridContainer\">"; /* id added BY BHUMITA ON 29/07/2025 */
                                     $_blldetail = new bll_customerwiseitempreservationpricelistdetail();
                                     $detailHtml = $_blldetail->pageSearch();
                                    if($detailHtml)
                                        $output.=$detailHtml; 
                                    /* REMOVED ADD DETAIL BUTTON BY BHUMITA ON 29/07/2025 */
                                $output.="
                        </div>
                    </div>
 ...
 class dal_customerwiseitempreservationpricelistmaster                         
{
    public function dbTransaction($_mdl)                     
    {
        ...
        try {
            if($_mdl->_customer_wise_item_preservation_price_list_id=="") {
                $_mdl->_customer_wise_item_preservation_price_list_id=0;
            }
            ...
            $_pre=$_dbh->prepare("CALL customer_wise_item_preservation_price_list_master_transaction (@p0,?,?,?,?,?,?,?,?,?,?,?) ");
                ...
                if($_mdl->_ajaxAdd==1) {
                    $_mdl->_ajaxAdd=0;
                    if($_mdl->_customer_wise_item_preservation_price_list_id)
                        $return_id=$_mdl->_customer_wise_item_preservation_price_list_id;
                    else 
                        $return_id=0;
                    echo $return_id;
                    exit;
                ...
                errorHandling($e, "customer_wise_item_preservation_price_list_id", $tbl_customer_wise_item_preservation_price_list_master, $ajax, $_mdl->_customer_wise_item_preservation_price_list_id);
    ...
    if(is_array($_bll->_mdl->generator_fields_names) && !empty($_bll->_mdl->generator_fields_names)){
        ...
        foreach($_bll->_mdl->generator_fields_names as $i=>$fieldname)
        {
            ...
            if($_bll->_mdl->generator_fields_types[$i]=="file") {
                $upload_dir = UPLOAD_DIR ."customer_wise_item_preservation_price_list_master/";
                $upload_path = UPLOAD_PATH ."customer_wise_item_preservation_price_list_master/";
                $url_fieldname = $fieldname . "_url";
