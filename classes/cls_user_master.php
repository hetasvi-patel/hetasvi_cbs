...
{   
    ...
    public function __construct() {
        ...
        $select->bindParam(1,  $tbl_user_master);
        $select->execute();
...
{   
     ...
     public function pageSearch()
    {
        ...
        $sql="CAll csms_search_detail('t.*, t4.value as status, u1.person_name as created_by, u2.person_name as modified_by, c1.company_name as company_id','tbl_user_master t LEFT JOIN view_status_type t4 ON t.status=t4.id LEFT JOIN tbl_user_master u1  ON t.created_by=u1.user_id  LEFT JOIN tbl_user_master u2  ON t.modified_by=u2.user_id  LEFT JOIN tbl_company_master c1  ON t.company_id=c1.company_id ','".$company_query."')";
        echo "<!-- Filter row -->
        ...
        if($canUpdate) {
        $_grid.="<form  method=\"post\" action=\"frm_user_master.php\" style=\"display:inline; margin-rigth:5px;\">
        ...
        if($canDelete && $_rs["user_role"]==3) { // ADDED USER_ROLE CONDTION BY BHUMITA ON 10/09/2025 
        $_grid.="<form  method=\"post\" action=\"classes/cls_user_master.php\" style=\"display:inline;\">
        ...
        try {
            ...
            $_pre=$_dbh->prepare("CALL user_master_transaction (@p0,?,?,?,?,?,?,?,?,?,?,?,?) ");
                ...
                if($_mdl->_transactionmode=="I") {
                    ...
                    $_mdl->_user_id=$insertedId;
                }
                if($_mdl->_ajaxAdd==1) {
                    $_mdl->_ajaxAdd=0;
                    if($_mdl->_user_id)
                        $return_id=$_mdl->_user_id;
                    else 
                        $return_id=0;
                    echo $return_id;
                    exit;
                ...
                errorHandling($e,"user_id",$tbl_user_master,$ajax,$_mdl->_user_id);
    ...
    if(is_array($_bll->_mdl->generator_fields_names) && !empty($_bll->_mdl->generator_fields_names)){
        ...
        foreach($_bll->_mdl->generator_fields_names as $i=>$fieldname)
        {
            ...
            if($_bll->_mdl->generator_fields_types[$i]=="file") {
                $upload_dir = UPLOAD_DIR ."user_master/";
                $upload_path = UPLOAD_PATH ."user_master/";
                $url_fieldname = $fieldname . "_url";
