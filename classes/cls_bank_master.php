...
{   
    ...
    public function __construct()    
    {
        $this->_mdl =new mdl_bankmaster(); 
        $this->_dal =new dal_bankmaster();
    }
    ...
    public function dbTransaction()
    {
       ...
       {
            header("Location:../srh_bank_master.php");
       }
       ...
       {
            header("Location:../frm_bank_master.php");
       }
     ...
     public function pageSearch()
    {
        ...
        $sql="CAll csms_search_detail('t.*, t5.value as status, u1.person_name as created_by, u2.person_name as modified_by, c1.company_name as company_id','tbl_bank_master t LEFT JOIN view_status_type t5 ON t.status=t5.id LEFT JOIN tbl_user_master u1  ON t.created_by=u1.user_id  LEFT JOIN tbl_user_master u2  ON t.modified_by=u2.user_id  LEFT JOIN tbl_company_master c1  ON t.company_id=c1.company_id ','".$company_query."')";
        echo "<!-- Filter row -->
        ...
        if($canUpdate) {
        $_grid.="<form  method=\"post\" action=\"frm_bank_master.php\" style=\"display:inline; margin-rigth:5px;\">
            <i class=\"fa fa-edit update\" style=\"cursor: pointer;\"></i>
            <input type=\"hidden\" name=\"bank_id\" value=\"".$_rs["bank_id"]."\" />
            <input type=\"hidden\" name=\"transactionmode\" value=\"U\"  />
            </form>";
        }
        if($canDelete) { 
        $_grid.="<form  method=\"post\" action=\"classes/cls_bank_master.php\" style=\"display:inline;\">
 ...
 class dal_bankmaster                         
{
    public function dbTransaction($_mdl)                     
    {
        ...
        try {
            ...
            $_dbh->exec("set @p0 = ".$_mdl->_bank_id);
            $_pre=$_dbh->prepare("CALL bank_master_transaction (@p0,?,?,?,?,?,?,?,?,?,?,?) ");
                ...
                if($_mdl->_transactionmode=="I") {
                    ...
                    $_mdl->_bank_id=$insertedId;
                }
                if($_mdl->_ajaxAdd==1) {
                    $_mdl->_ajaxAdd=0;
                    if($_mdl->_bank_id)
                        $return_id=$_mdl->_bank_id;
                    else 
                        $return_id=0;
                    echo $return_id;
                    exit;
                ...
                errorHandling($e,"bank_id",$tbl_bank_master, $ajax, $_mdl->_bank_id);
            }
    ...
    {
        global $_dbh;
        $_pre=$_dbh->prepare("CALL bank_master_fillmodel (?) ");
        $_pre->bindParam(1,$_REQUEST["bank_id"]);
        $_pre->execute();
        $_rs=$_pre->fetchAll(); 
        if(!empty($_rs)) {
            if(is_array($_mdl->generator_fields_names) && !empty($_mdl->generator_fields_names)){
    ...
    if(is_array($_bll->_mdl->generator_fields_names) && !empty($_bll->_mdl->generator_fields_names)){
        ...
        foreach($_bll->_mdl->generator_fields_names as $i=>$fieldname)
        {
            ...
            if($_bll->_mdl->generator_fields_types[$i]=="file") {
                $upload_dir = UPLOAD_DIR ."bank_master/";
                $upload_path = UPLOAD_PATH ."bank_master/";
                $url_fieldname = $fieldname . "_url";
