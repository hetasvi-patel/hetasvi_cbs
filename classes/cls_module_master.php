...
{   
    ...
    public function __construct()    
    {
        $this->_mdl =new mdl_modulemaster(); 
        $this->_dal =new dal_modulemaster();
    }
    ...
    public function dbTransaction()
    {
       ...
       {
            header("Location:../srh_module_master.php");
       }
       ...
       {
            header("Location:../frm_module_master.php");
       }
    ...
    public function pageSearch()
    {
        ...
        global $canUpdate;
        global $canDelete;
       // $company_query=str_replace("company_id","t.company_id",COMPANY_QUERY); // COMMENTED BY BHUMITA ON 25/09/2025
        $sql="CAll csms_search_detail('t.*','tbl_module_master t','')"; // REMOVED $company_query BY BHUMITA ON 25/09/2025
        echo "<!-- Filter row -->
        ...
        if($canUpdate) {
        $_grid.="<form  method=\"post\" action=\"frm_module_master.php\" style=\"display:inline; margin-rigth:5px;\">
            <i class=\"fa fa-edit update\" style=\"cursor: pointer;\"></i>
            <input type=\"hidden\" name=\"module_id\" value=\"".$_rs["module_id"]."\" />
            <input type=\"hidden\" name=\"transactionmode\" value=\"U\"  />
            </form>";
        }
        if($canDelete) { 
        $_grid.="<form  method=\"post\" action=\"classes/cls_module_master.php\" style=\"display:inline;\">
    ...
    public function checkDuplicate() {
        ...
        try {
           $sql="CAll csms_check_duplicate('".$column_name."','".$column_value."','".$id_name."','".$id_value."','".$table_name."',NULL,@is_duplicate,".$scope_field_name.",".$scope_field_value.")"; // PASSED NULL FOR COMPANY ID BY BHUMITA ON 25/09/2025
 ...
 class dal_modulemaster                         
{
    public function dbTransaction($_mdl)                     
    {
        ...
        try {
            ...
            $_dbh->exec("set @p0 = ".$_mdl->_module_id);
            $_pre=$_dbh->prepare("CALL module_master_transaction (@p0,?,?,?,?) ");
                ...
                if($_mdl->_transactionmode=="I") {
                    ...
                    $_mdl->_module_id=$insertedId;
                }
                if($_mdl->_ajaxAdd==1) {
                    $_mdl->_ajaxAdd=0;
                    if($_mdl->_module_id)
                        $return_id=$_mdl->_module_id;
                    else 
                        $return_id=0;
                    echo $return_id;
                    exit;
                ...
                errorHandling($e,"module_id",$tbl_module_master, $ajax);
    ...
    {
        global $_dbh;
        $_pre=$_dbh->prepare("CALL module_master_fillmodel (?) ");
        $_pre->bindParam(1,$_REQUEST["module_id"]);
        $_pre->execute();
        $_rs=$_pre->fetchAll(); 
        if(!empty($_rs)) {
            if(is_array($_mdl->generator_fields_names) && !empty($_mdl->generator_fields_names)){
    ...
    if(is_array($_bll->_mdl->generator_fields_names) && !empty($_bll->_mdl->generator_fields_names)){
        ...
        foreach($_bll->_mdl->generator_fields_names as $i=>$fieldname)
        {
            ...
            if($_bll->_mdl->generator_fields_types[$i]=="file") {
                $upload_dir = UPLOAD_DIR ."module_master/";
                $upload_path = UPLOAD_PATH ."module_master/";
                $url_fieldname = $fieldname . "_url";
