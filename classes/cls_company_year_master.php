...
{   
     ...
     public function pageSearch()
    {
        ...
        $sql="CAll csms_search_detail('t.*, t1.value as company_year_type, u1.person_name as created_by, u2.person_name as modified_by, c1.company_name as company_id','tbl_company_year_master t LEFT JOIN view_company_year_type t1 ON t.company_year_type=t1.id LEFT JOIN tbl_user_master u1  ON t.created_by=u1.user_id  LEFT JOIN tbl_user_master u2  ON t.modified_by=u2.user_id  LEFT JOIN tbl_company_master c1  ON t.company_id=c1.company_id ','".$company_query."')";
        echo "<!-- Filter row -->
        ...
        $_grid.="<tr>";
        if($canDelete) { // $canUpdate removed BY BHUMITA ON 10/07/2025
        $_grid.="<td data-label=\"Action\">";
        }
       /* EDIT ICON REMOVED BY BHUMITA ON 10/07/2025 */
        if($canDelete) { 
        $_grid.="<form  method=\"post\" action=\"classes/cls_company_year_master.php\" style=\"display:inline;\">
    ...
    public function checkDuplicate() {
        ...
        if(isset($_POST["scope_field_name"]))

            $scope_field_name="'".$_POST["scope_field_name"]."'";

        if(isset($_POST["scope_field_value"]))
    ...
    /* ADDED BY BHUMITA ON 12/07/2025 */

    function getLastYear() {

        $company_year_type = $_POST["company_year_type"];

        global $_dbh;

        global $tbl_company_year_master;

        $order = ($company_year_type == "2") ? "MIN" : "MAX";

        $sql="CAll csms_search_detail('{$order}(start_date) as last_start','".$tbl_company_year_master."',' and company_year_type = ".$company_year_type." ".COMPANY_QUERY."')";
...

{
...

    {
        ...
        try {
            ...
            if($_mdl->_company_year_id=="") {

                $_mdl->_company_year_id=0;
            ...
            $_pre=$_dbh->prepare("CALL company_year_master_transaction (@p0,?,?,?,?,?,?,?,?,?) ");
                ...
                if($_mdl->_transactionmode=="D") {
                    ...
                    if(isset($_SESSION["sess_company_year_id"]) && $_SESSION["sess_company_year_id"]==$_mdl->_company_year_id) {
                ...
                if($_mdl->_transactionmode=="I") {
                    ...
                    $_mdl->_company_year_id=$insertedId;
                ...
                if($_mdl->_ajaxAdd==1) {
                    ...
                    if($_mdl->_company_year_id)
                        ...
                        $return_id=$_mdl->_company_year_id;
                ...
                errorHandling($e,"company_year_id",$tbl_company_year_master,$ajax, $_mdl->_company_year_id);
            }
    ...
    if(is_array($_bll->_mdl->generator_fields_names) && !empty($_bll->_mdl->generator_fields_names)){
...

        {
            ...
            if($_bll->_mdl->generator_fields_types[$i]=="file") {
                ...
                $upload_dir = UPLOAD_DIR ."company_year_master/";

                $upload_path = UPLOAD_PATH ."company_year_master/";
