...
{   
     ...
     public function pageSearch()
    {
        ...
        $sql="CAll csms_search_detail('t.*, u1.person_name as created_by, u2.person_name as modified_by, c1.company_name as company_id','tbl_country_master t LEFT JOIN tbl_user_master u1  ON t.created_by=u1.user_id  LEFT JOIN tbl_user_master u2  ON t.modified_by=u2.user_id  LEFT JOIN tbl_company_master c1  ON t.company_id=c1.company_id ','".$company_query."')";
        echo "<!-- Filter row -->
    ...
    public function dbTransaction($_mdl)                     
    {
        ...
        try {
            ...
            $_pre=$_dbh->prepare("CALL country_master_transaction (@p0,?,?,?,?,?,?,?) ");
                ...
                errorHandling($e,"country_id",$tbl_country_master, $ajax, $_mdl->_country_id);
    ...
    if(is_array($_bll->_mdl->generator_fields_names) && !empty($_bll->_mdl->generator_fields_names)){
        ...
        foreach($_bll->_mdl->generator_fields_names as $i=>$fieldname)
        {
            ...
            if($_bll->_mdl->generator_fields_types[$i]=="file") {
                $upload_dir = UPLOAD_DIR ."country_master/";
                $upload_path = UPLOAD_PATH ."country_master/";
                $url_fieldname = $fieldname . "_url";
