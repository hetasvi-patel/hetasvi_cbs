...
{   
     ...
     public function pageSearch()
    {
        ...
        $sql="CAll csms_search_detail('t.*, t2.state_name as state_id,c.country_name as country_id, u1.person_name as created_by, u2.person_name as modified_by, c1.company_name as company_id','tbl_city_master t LEFT JOIN tbl_state_master t2 ON t.state_id=t2.state_id LEFT JOIN tbl_country_master c ON t.country_id=c.country_id LEFT JOIN tbl_user_master u1  ON t.created_by=u1.user_id  LEFT JOIN tbl_user_master u2  ON t.modified_by=u2.user_id  LEFT JOIN tbl_company_master c1  ON t.company_id=c1.company_id ','".$company_query."')"; // COUNTRY NAME FETCH FUNCTIONALITY ADDED IN THIS QUERY BY BHUMITA ON 13/07/2025
        echo "<!-- Filter row -->
    ...
    public function fetchCountry() {
        ...
        global $tbl_state_master;
        $state_id = $_POST['state_id'];
        $columns = 's.country_id, c.country_name';
        $tableName = $tbl_state_master.' s JOIN '.$tbl_country_master.' c ON s.country_id = c.country_id';
        $whereCondition = " and s.state_id = $state_id";
    ...
    public function getForm($transactionmode="I",$popup=false,$label_classes="col-12 col-sm-3 col-md-2 col-lg-2 col-xl-2 col-xxl-1", $field_classes="col-12 col-sm-6 col-md-4 col-lg-4 col-xl-3 col-xxl-2") {
        ...
        $state_menu_id = getMenuId("frm_state_master.php");
        $canAddState = userHasRight(USER_ID, $state_menu_id, 'add'); 
        /* \ADDED BY BHUMITA ON 13/07/2025 */
        ...
        if(is_array($this->_mdl->generator_fields_names) && !empty($this->_mdl->generator_fields_names)){
            ...
            {   
                ...
                if($fieldname=="country_name") {
                    continue; // Skip country_name as it is not a form field
                }
                /* \ADDED BY BHUMITA ON 13/07/2025 */
                ...
                $add_modal_cls="";  //ADDED BY BHUMITA 13/07/2025
                ...
                switch($this->_mdl->generator_fields_types[$i]) {
                    ...
                    case "number":
                    case "select":
                        $value="";$field_str="";$cls="";$flag=0;$disabled_value=""; // $disabled_value added by BHUMITA on 13/07/2025
                            ...
                            if($this->_mdl->generator_fields_types[$i]=="number") {
                                ...
                                if(!empty($this->_mdl->generator_dropdown_table[$i]) && !empty($this->_mdl->generator_label_column[$i]) && !empty($this->_mdl->generator_value_column[$i])) {
                                    ...
                                    if(!$popup && $canAddState && $fieldname=="state_id") {
                                ...
                                if($flag==0) {
                                    ...
                                    /* \ADDED BY BHUMITA 07/07/2025 */
                                    $field_str.=addInput($this->_mdl->generator_fields_types[$i],$fieldname,$value,$required_str,$disabled_str,$cls,$duplicate_str,$chk_str,$this->_mdl->generator_fields_labels[$i],$disabled_value).$error_container.$img_str; // Last parameter $disabled_value added by BHUMITA on 13/07/2025
                                }
                            }
                        break;
                    case "hidden":
                    ...
                    case "textarea":
                        ...
                        $field_str.=addTextArea($fieldname,$value,$required_str,$disabled_str,$cls,$duplicate_str,$this->_mdl->generator_fields_labels[$i]).$error_container;
                        break;
                    default:
                        break;
                } //switch ends
                 if(empty($this->_mdl->generator_after_detail) || (!empty($this->_mdl->generator_after_detail) && !in_array($fieldname,$this->_mdl->generator_after_detail))) {
                    ...
                    $output.=$lbl_str;
                    if($field_str) {
                    $output.='<div class="'.$field_layout_classes.' '.$add_modal_cls.'">'; //$add_modal_cls ADDED BY BHUMITA 12/07/2025
                    $output.=$field_str;
                    $output.='</div>';
                    }
                    if($table_layout=="vertical" && $this->_mdl->generator_fields_types[$i]!="hidden") {
        ...
        try {
            ...
            $_pre=$_dbh->prepare("CALL city_master_transaction (@p0,?,?,?,?,?,?,?,?,?) ");
                ...
                if($_mdl->_transactionmode=="I") {
                    ...
                    $_mdl->_city_id=$insertedId;
                }
                if($_mdl->_ajaxAdd==1) {
                    $_mdl->_ajaxAdd=0;
                    if($_mdl->_city_id)
                        $return_id=$_mdl->_city_id;
                    else 
                        $return_id=0;
                    echo $return_id;
                    exit;
            ...
            errorHandling($e, "city_id", $tbl_city_master, $ajax, $_mdl->_city_id);
        }
    ...
    public function fillModel($_mdl)
    {
        ...
        if(!empty($_rs)) {
            if(is_array($_mdl->generator_fields_names) && !empty($_mdl->generator_fields_names)){
                ...
                $_mdl->_country_name = $_rs[0]['country_name'] ?? ''; // ADDED BY BHUMITA ON 13/07/2025
            }
    ...
    if(is_array($_bll->_mdl->generator_fields_names) && !empty($_bll->_mdl->generator_fields_names)){
        ...
        foreach($_bll->_mdl->generator_fields_names as $i=>$fieldname)
        {
            ...
            if($_bll->_mdl->generator_fields_types[$i]=="file") {
                $upload_dir = UPLOAD_DIR ."city_master/";
                $upload_path = UPLOAD_PATH ."city_master/";
                $url_fieldname = $fieldname . "_url";
