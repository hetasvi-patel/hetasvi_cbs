...
{   
    ...
    public function __construct() {
        ...
        $select->bindParam(1,  $tbl_menu_master);
        $select->execute();
...
{   
     ...
     public function pageSearch()
    {
        ...
        $sql="CAll csms_search_detail('t1.module_name as module_id, t.menu_name, t.menu_text, t.menu_url, t.tab_index, t.is_display, t.menu_id','tbl_menu_master t LEFT JOIN tbl_module_master t1 ON t.module_id=t1.module_id','".$company_query."')";
        echo "<!-- Filter row -->
                ...
                foreach($this->_mdl->generator_fields_names as $i=>$fieldname)
                {
                    ...
                    $hstr.="<div class=\"col-auto\">";
                    $hstr.="<input type=\"text\" class=\"form-control ".$extracls."\" placeholder=\"Search ".$this->_mdl->generator_fields_labels[$i]."\" data-index=\"".$k."\" />";
                    $hstr.="</div>";
                }
                echo $hstr;echo "</div>";
        echo "
        ...
        <thead>
            ...
            foreach($this->_mdl->generator_fields_names as $i=>$fieldname)
            {
                ...
                if($fieldname=="company_id") {
                    if(COMPANY_ID==ADMIN_COMPANY_ID) {
                        $hstr.= "<th> ".$this->_mdl->generator_fields_labels[$i]." </th>"; 
                    }
                    continue;
                }
                if($this->_mdl->generator_fields_types[$i]=="file") {
                ...
                $hstr.="<th>";
                $hstr.=$this->_mdl->generator_fields_labels[$i];
                $hstr.="</th>";
            }
        echo $hstr;
               echo "</tr>
        ...
        if($canUpdate) {
        $_grid.="<form  method=\"post\" action=\"frm_menu_master.php\" style=\"display:inline; margin-rigth:5px;\">
        ...
        if($canDelete) { 
        $_grid.="<form  method=\"post\" action=\"classes/cls_menu_master.php\" style=\"display:inline;\">
        ...
        foreach($this->_mdl->generator_fields_names as $i=>$fieldname)
        {
            ...
            if($fieldname=="company_id") {
                if(COMPANY_ID==ADMIN_COMPANY_ID) {
                    $_grid.= "<td data-label=\"".$this->_mdl->generator_fields_labels[$i]."\"> ".$_rs[$fieldname]." </td>"; 
                }
                continue;
            }
            ...
            } else if($this->_mdl->generator_fields_types[$i]=="checkbox") {
                $fieldvalue=($_rs[$fieldname]==1) ? "Yes" : "No";
            } else if($this->_mdl->generator_fields_types[$i]=="radio") {
                $fieldvalue=($_rs[$fieldname]==1) ? "Yes" : "No";
            } else {
                $fieldvalue=$_rs[$fieldname];
            }
            $_grid.="<td data-label=\"".$this->_mdl->generator_fields_labels[$i]."\">";
            $_grid.=$fieldvalue;
            $_grid.="</td>";
        }
        $_grid.= "</tr>\n";
        ...
        try {
            ...
            $_pre=$_dbh->prepare("CALL menu_master_transaction (@p0,?,?,?,?,?,?,?,?) ");
                ...
                if($_mdl->_transactionmode=="I") {
                    ...
                    $_mdl->_menu_id=$insertedId;
                }
                if($_mdl->_ajaxAdd==1) {
                    $_mdl->_ajaxAdd=0;
                    if($_mdl->_menu_id)
                        $return_id=$_mdl->_menu_id;
                    else 
                        $return_id=0;
                    echo $return_id;
                    exit;
    ...
    if(is_array($_bll->_mdl->generator_fields_names) && !empty($_bll->_mdl->generator_fields_names)){
        ...
        foreach($_bll->_mdl->generator_fields_names as $i=>$fieldname)
        {
            ...
            if($_bll->_mdl->generator_fields_types[$i]=="file") {
                $upload_dir = UPLOAD_DIR ."menu_master/";
                $upload_path = UPLOAD_PATH ."menu_master/";
                $url_fieldname = $fieldname . "_url";
