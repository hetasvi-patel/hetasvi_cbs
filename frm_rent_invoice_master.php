...
document.getElementById("btn_add").addEventListener("click", function (event) {
    ...
    setTimeout(function(){
        ...
        if(invalidInputs.length>0) {
            // Don't proceed if there are invalid inputs
        } else {
            // Convert jsonData to JSON string for form submission
            const jsonDataString = JSON.stringify(jsonData);
            document.getElementById("detail_records").value = jsonDataString;
            
            // Convert deletedData to JSON string for form submission
        ...
        success: function (invoiceData) {
            ...
            if (!invoiceData || invoiceData.length === 0) {
                ...
                invoiceData.forEach((data, index) => {
                    ...
                    // Add to jsonData for saving
                    const detailRecord = {
                        rent_invoice_detail_id: 0, // Will be set on insert
                        rent_invoice_id: 0, // Will be set when master is saved
                        description: `${data.item} (${data.lot_no}) - ${data.storage_duration}`,
                        qty: data.qty,
                        unit: data.unit,
                        weight: data.weight,
                        rate_per_unit: data.rent_per_storage_duration,
                        amount: data.amount,
                        remark: `Charges from ${data.charges_from} to ${data.charges_to}`,
                        detailtransactionmode: "I",
                        // Additional fields for generated data
                        ...
                        outward_date: data.out_date,
                        charges_from: data.charges_from,
                        charges_to: data.charges_to,
                ...
                Swal.fire({
                    ...
                    text: `${invoiceData.length} records have been generated and will be saved with the invoice.`,
                    showConfirmButton: true
                });
            }
