...
?>
  ...
  <div class="content-wrapper">
            ...
            <!-- .box-footer -->
              <div class="box-footer">
                ...
                <input class="btn btn-success" type="button" id="btn_add" name="btn_add" value= "Save">
                  <input type="button" class="btn btn-primary" id="btn_search" name="btn_search" value="Search" onclick="window.location='srh_customer_wise_item_preservation_price_list_master.php'">
                ...
                <input class="btn btn-secondary" type="button" id="btn_reset" name="btn_reset" value="Reset" onclick="document.getElementById('masterForm').reset();" >
                   <input type="button" class="btn btn-default" id="btn_frm" name="btn_frm" value="Cancel" onclick="window.location='frm_customer_wise_item_preservation_price_list_master.php'">
              </div>
              <!-- /.box-footer -->
        </form>
        <!-- form end -->
    ...
    <!-- REMOVED DETAIL MODAL HTML AND CODING BY BHUMITA ON 29/07/2025 -->
    <!-- /.container -->
  </div>
  <!-- /.content-wrapper -->
  <?php
...
document.addEventListener("DOMContentLoaded", function () {    
    ...
    const masterId = $('#customer_wise_item_preservation_price_list_id').val();
    ...
    if (masterId > 0 && itemId && customerId) {
        fetchDetailRecords(itemId, masterId, customerId);
    }
    function populateMasterForm(data) {
        $('#customer_wise_item_preservation_price_list_id').val(data.customer_wise_item_preservation_price_list_id);
        $('#rent_per_kg_month').val(data.rent_per_kg_month);
        $('#rent_per_kg_season').val(data.rent_per_kg_season);
        $('#transactionmode').val('U');
    }
    function resetMasterForm() {
        $('#customer_wise_item_preservation_price_list_id').val(0);
        $('#rent_per_kg_month').val('');
        $('#rent_per_kg_season').val('');
        $('#transactionmode').val('I');
    }
    function fetchDetailRecords(itemId, masterId, customerId) {
        //console.log(itemId+" "+masterId);
        $.ajax({
            url: 'classes/cls_customer_wise_item_preservation_price_list_detail.php',
            type: 'POST',
            data: {
    ...
    function checkMasterRecord(itemId,masterId,customerId) {
        $.ajax({
            url: 'classes/cls_customer_wise_item_preservation_price_list_master.php',
            type: 'POST',
            dataType: 'json',
            data: {
            ...
            success: function (response) {
                ...
                if (response.exists) {
                    ...
                    fetchDetailRecords(itemId, response.data.customer_wise_item_preservation_price_list_id,customerId);
                    $('#transactionmode').val('U');
                } else if (response.prevYearData) {
       
                    const prevYearData = response.prevYearData;
                    populateMasterForm(prevYearData);
                    fetchDetailRecords(itemId, prevYearData.customer_wise_item_preservation_price_list_id,customerId);

                    // Always insert for new year
                    $('#customer_wise_item_preservation_price_list_id').val(0);
                    $('#transactionmode').val('I');
                }
                else if (response.prevItemData) {
       
                    const prevItemData = response.prevItemData;
                    populateMasterForm(prevItemData);
                    fetchDetailRecords(itemId, 0,customerId);
    ...
    $('#item_id, #customer_id').on('change', function () {
        const itemId = $('#item_id').val();
        const customerId = $('#customer_id').val();
        const masterId = $('#customer_wise_item_preservation_price_list_id').val() || 0;

        if (itemId && customerId) {
            checkMasterRecord(itemId, masterId, customerId);
        } else if (!itemId && !customerId) {
            resetForm();
        }
    });
    function collectChangedDetails() {
        ...
        gridRows.each(function () {
            ...
            if (rentPerQtyMonth !== originalMonth ||
                ...
                detailRecords.push({
                    customer_wise_item_preservation_price_list_detail_id: detailId,
                    customer_wise_item_preservation_price_list_id: masterId,
...
<?php
    frmAlert("frm_customer_wise_item_preservation_price_list_master.php");
?>
<?php
